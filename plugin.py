...
# Backup mode (alimentation controlee du WAN2) ---------------------------------------------------
    def manage_backup_power(self):
        ...
        remaining_warmup = self.wan2_warmup - (now - self.wan2_power_since)
        if remaining_warmup > 0:
            self.wan2_status_text = "ON, warmup {}s restantes ({})".format(int(remaining_warmup), self.wan2_active_reason)
            return False, 0

        star_ok, star_latency = check_route(self.star_ip, "192.168.2.1")

        self.wan2_status_text = "{} - {} ms ({})".format(
            "OK" if star_ok else "DOWN", star_latency, self.wan2_active_reason
        )
