...
# Version:    2.0.1: ER605 native WAN1 state, traceroute and ping hardening
# Version:    2.0.2: WAN1 tested through a WAN1-pinned IP (ER605 policy route)
# Version:    2.0.3: recovery probe - the ER605 never falls back to WAN1 on its own
# Version:    2.0.4: ER605 API disabled - both WANs tested through pinned routes
# Version:    2.0.5: WAN1 stable cutoff 600s -> 180s (load balancing overlap)
# Version:    2.0.6: ER605 credentials removed - no authenticated call left

"""
<plugin key="WanCheck" name="WAN Check WAN1/WAN2" author="Erwan" version="2.0.6">
    <params>
        ...
        <param field="Address" label="ER605 IP (ping de disponibilite, laisser vide pour desactiver)" width="200px" required="false" default=""/>
    </params>
</plugin>
"""
...
class BasePlugin:
    def __init__(self):
        ...
        # 3 minutes de WAN1 ok avant de couper WAN2. Sans Link Backup sur l'ER605,
        # les deux WAN actifs en meme temps sont repartis en load balancing : une
        # partie du trafic sortirait par le CGNAT Starlink pendant toute cette
        # fenetre. On la garde courte, quitte a rallumer au cycle suivant si la
        # fibre se revele instable.
        self.wan1_stable_cutoff = 180
        ...
        self.wan2_test_timeout = 900     # 15 min max avant de declarer le test en echec
        ...
        # sonde de retour : l'ER605 ne repasse jamais seul sur WAN1 apres un
        # basculement (comportement reproduit en debranchant/rebranchant WAN1).
        # Tant que Starlink est alimente, l'ER605 route tout par lui et le test
        # WAN1 ne peut pas voir la fibre revenir : le plugin s'auto-entretient
        # dans l'etat de secours. On coupe donc brievement la prise pour rendre
        # la fibre observable, puis on decide de rallumer ou non.
        self.wan2_probe_interval = 1800  # 30 min entre deux tentatives de retour
        self.wan2_probe_duration = 90    # coupure assez longue pour que l'ER605 rebascule
        self.wan2_probe_until = None     # fin de la fenetre de sonde en cours
        self.last_wan2_probe = 0
        ...
        # L'API n'est pas exploitable sur ce firmware (404 sur /cgi-bin/luci/) et
        # chaque tentative de login risque de declencher la protection
        # anti-bruteforce du routeur - au point de verrouiller l'acces admin.
        # Les deux WAN sont de toute facon testes par ping+traceroute via leurs
        # routes epinglees (Policy Routing sur l'ER605), ce qui donne un signal
        # plus fort que l'etat natif du lien : la connectivite de bout en bout.
        # Repasser a True si le bon endpoint est retrouve un jour.
        self.er605_api_enabled = False
        # apres un echec de login, on attend avant de retenter plutot que de
        # reessayer a chaque cycle : des logins repetes en rafale (ex: 1x/min
        # pendant 10 min) peuvent declencher une protection anti-bruteforce sur le
        # routeur, qui se manifeste alors par des echecs en cascade (ex: HTTP 404)
        self.er605_login_cooldown = 180  # 3 min avant de retenter apres un echec
        self.er605_login_cooldown_until = 0
    ...
    def onStart(self):
        ...
        # WAN1 est teste via une IP epinglee sur WAN1 par une regle de Policy
        # Routing de l'ER605. Le test reste donc valable meme quand l'ER605 route
        # tout par Starlink : c'est ce qui permet de voir la fibre revenir pendant
        # un backup en cours, au lieu de rester bloque (voir plus bas).
        self.main_ip = "8.8.4.4"
        self.star_ip = "1.1.1.1"

        # passerelles WAN vues depuis le LAN, utilisees pour savoir quel lien
        # porte reellement le trafic (saut 2 du traceroute, apres l'ER605)
        self.wan1_gateway = "192.168.1.1"   # box fibre
        self.wan2_gateway = "192.168.2.1"   # Starlink
        ...
        # Identifiants retires des parametres du plugin : plus aucun appel
        # authentifie n'est emis (voir er605_api_enabled). Ils ne sont relus ici
        # que si les champs sont un jour readjoutes au bloc <params> ci-dessus.
        try:
        ...
        if self.er605_api_enabled and self.wan2_mode == "Backup" and self.er605_host:
    ...
    def check_all(self):
        ...
        if now - self.last_connectivity_check >= self.connectivity_check_interval:
            ...
            # WAN1 : ping vers l'IP epinglee sur WAN1 + verification que le trafic
            # traverse bien la box fibre (192.168.1.1). Le traceroute joue ici un
            # double role : il confirme que la fibre atteint internet, et que la
            # regle de Policy Routing est bien active - sans elle le paquet
            # sortirait par le WAN courant et le test serait faussement vert.
            self.cached_main_ok, self.cached_main_latency = check_route(self.main_ip, self.wan1_gateway)

            # l'API ER605 n'est interrogee que quand elle sert reellement a quelque
            # chose : en continu pour FlowBalancing (seule source fiable pour WAN2,
            # vu le probleme de routage), ou seulement pendant le test quotidien en
            # mode Backup (le seul cas ou WAN1 reste actif pendant le test WAN2,
            # donc le seul ou le ping/traceroute du Pi ne suffit pas). Pendant une
            # vraie coupure ("backup"), le ping suffit -> pas d'appel API, pour ne
            # pas la solliciter inutilement pendant un incident reel.
            if self.wan2_mode != "Backup" or self.wan2_active_reason == "dailytest":
                self.cached_er605_states = self.er605_online_states()
            else:
                self.cached_er605_states = None

            if self.wan2_mode != "Backup":
                self.cached_flow_star_ok, _ = check_route(self.star_ip, self.wan2_gateway)

        main_ok, main_latency = self.cached_main_ok, self.cached_main_latency
        er605_states = self.cached_er605_states

        # quand l'etat natif de l'ER605 est disponible, il prime sur le ping : le
        # ping dit seulement "internet repond", sans distinguer par quel lien, donc
        # il reste vert apres un failover vers WAN2
        ...
        if main_ok != self.last_main_state or star_ok != self.last_star_state:
            ...
            if self.pending_count >= 4:
                ...
                elif main_ok and not self.valid_main_state and self.wan2_active_reason == "backup":
                    # WAN1 vient de revenir pendant une vraie coupure en cours :
                    # notification immediate, deliverable tout de suite puisque
                    # WAN1 fonctionne de nouveau (contrairement a une eventuelle
                    # alerte d'echec WAN2, invivrable tant que rien ne marche)
                    Send_Notifications(
                        self,
                        "WAN ALERT - WAN1 (MAIN) revenu",
                        "WAN1 (MAIN) OK a {}".format(time.strftime("%H:%M")),
                        1
                    )
...
# Backup mode (alimentation controlee du WAN2) ---------------------------------------------------
    def manage_backup_power(self, er605_states=None):
        ...
        # sonde de retour (voir __init__) : uniquement pendant une vraie coupure,
        # jamais pendant le test quotidien
        if self.wan2_active_reason == "backup":
            if self.wan2_probe_until is not None:
                if now < self.wan2_probe_until:
                    self.wan2_status_text = "OFF (sonde de retour en cours)"
                    return False

                self.wan2_probe_until = None
                # mesure directe : on a cree les conditions pour observer WAN1,
                # inutile de passer par la confirmation sur 4 battements
                wan1_back, _ = check_route(self.main_ip, self.wan1_gateway)

                if wan1_back:
                    Domoticz.Log("Backup mode: WAN1 (MAIN) de nouveau operationnel, {} reste coupe".format(
                        self.wan2_label
                    ))
                    self.power_wan2(False, "retour de WAN1 (MAIN) detecte par la sonde")
                    return False

                Domoticz.Log("Backup mode: WAN1 (MAIN) toujours DOWN, {} rallume".format(self.wan2_label))
                try:
                    switch_command(self.wan2_switch_idx, "On")
                except Exception as e:
                    Domoticz.Error("Error restoring {} power after probe: {}".format(self.wan2_label, str(e)))
                self.last_wan2_probe = now

            elif now - self.last_wan2_probe >= self.wan2_probe_interval:
                Domoticz.Log("Backup mode: sonde de retour, coupure de {} pendant {}s".format(
                    self.wan2_label, self.wan2_probe_duration
                ))
                try:
                    switch_command(self.wan2_switch_idx, "Off")
                except Exception as e:
                    Domoticz.Error("Error cutting {} power for probe: {}".format(self.wan2_label, str(e)))
                    return False
                self.wan2_probe_until = now + self.wan2_probe_duration
                self.wan2_status_text = "OFF (sonde de retour en cours)"
                return False
        ...
        if self.wan2_active_reason == "dailytest":
            # test quotidien : WAN1 reste actif pendant le test, donc l'ER605
            # continue de router par WAN1 tant qu'il ne le juge pas down -> le
            # ping/traceroute du Pi ne passera jamais reellement par WAN2. Seule
            # l'API ER605 (etat natif, deja recupere dans check_all) peut confirmer
            # que WAN2 fonctionne vraiment ; sinon on retombe sur le ping/traceroute.
            er605_wan2_up = er605_states.get("WAN2") if er605_states else None
            if er605_wan2_up is not None:
                star_ok = er605_wan2_up
            else:
                star_ok, _ = check_route(self.star_ip, self.wan2_gateway)
        else:
            # vraie coupure ("backup") : WAN1 est reellement indisponible, donc le
            # trafic du Pi passe forcement par WAN2 si celui-ci fonctionne -> le
            # ping/traceroute classique suffit et est fiable dans ce cas precis.
            # On evite deliberement l'API ER605 ici : la solliciter pendant un
            # incident reel n'apporte rien et risque de la faire echouer/verrouiller
            # (voir is_er605_reachable/cooldown) au pire moment.
            star_ok, _ = check_route(self.star_ip, self.wan2_gateway)
        ...
        if star_ok and not self.wan2_test_success_notified:
            ...
            if self.wan2_active_reason == "backup":
                # pas de notification push ici : si WAN1 ET WAN2 sont down, aucun
                # canal (Telegram/Pushover) n'est de toute facon joignable - le
                # message resterait en file d'attente et n'arriverait qu'au retour
                # d'une connexion, decorrele de la situation reelle a ce moment-la
                # (voir "WAN1 revenu" ci-dessus, qui lui est livrable immediatement).
                # On garde juste une trace dans le log Domoticz pour le diagnostic.
    ...
    def check_switch_drift(self, now):
        # pendant une sonde, la prise est volontairement coupee alors que l'etat
        # attendu reste "On" : ne pas la corriger, sinon la sonde est annulee
        if self.wan2_probe_until is not None:
            return
    ...
    def er605_login(self):
        ...
        try:
            ...
            self.er605_stok = None
            self.er605_login_cooldown_until = time.time() + self.er605_login_cooldown
            raise
    ...
    def er605_call(self, path, method_name):
        if time.time() < self.er605_login_cooldown_until:
            raise Exception("ER605 login en cooldown apres un echec recent")
    ...
    def er605_online_states(self):
        ...
        if not self.er605_api_enabled or not self.er605_host:
            return None
        try:
            states = {}
    ...
    def power_wan2(self, on, reason):
        ...
        if on:
            ...
            self.wan2_test_failure_notified = False
            self.wan2_probe_until = None
            self.last_wan2_probe = time.time()  # premiere sonde dans wan2_probe_interval
...
# Other defs ---------------------------------------------------
def check_route(ip, expected_gateway=None):
    # expected_gateway=None : on se contente du ping (cas WAN1, qui emprunte la
    # route par defaut). Sinon on verifie en plus que le trafic sort bien par la
    # passerelle attendue, ce qui n'a de sens que pour un lien qui n'est pas la
    # route par defaut.
    try:
        # ping pour récupérer la latence
        ping_cmd = ["ping", "-c", "2", "-W", "2", ip]
        ping_result = subprocess.run(ping_cmd, stdout=subprocess.PIPE, text=True, timeout=15)
        ...
        if expected_gateway is None:
            return True, latency

        # traceroute court pour vérifier le WAN utilisé
        # -n : pas de resolution DNS (inutile et lente)
        # -q 1 -w 1 : une sonde par saut, 1s d'attente -> ne peut plus depasser le timeout
        trace_cmd = ["traceroute", "-n", "-q", "1", "-w", "1", "-m", "3", ip]
        trace_result = subprocess.run(trace_cmd, stdout=subprocess.PIPE, text=True, timeout=15)
